...
function getBooks(filterBy) {
    const books = (!filterBy) ? [...gBooksList] :
        gBooksList.filter(book => book.title.includes(filterBy.name) && book.listPrice.amount <= filterBy.price);
    return Promise.resolve(books);
}

function getBookById(bookId) {
    const book = gBooksList.find(book => book.id === bookId)
    return Promise.resolve(book)
}

function addBook(googleBook) {
    gBooksList = [...gBooksList , new Book(googleBook.title , googleBook.subtitle , googleBook.authors ,
      googleBook.publishedDate, googleBook.categories , googleBook.imageLinks.thumbnail  , googleBook.language , googleBook.pageCount , 
      googleBook.description )]
}

function getBookPriceString(price, currency) {
    switch (currency) {
        case 'USD':
            return '$' + price;
            break;
